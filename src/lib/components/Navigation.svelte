<script lang="ts">
  import { onMount } from "svelte";

  interface Props {
    darkMode: boolean;
    toggleTheme: () => void;
    showQR: boolean;
  }

  let { darkMode, toggleTheme, showQR = $bindable() }: Props = $props();
  let scrolled = $state(false);
  let mobileMenuOpen = $state(false);

  const navItems = [
    { name: "Home", href: "#home" },
    { name: "About", href: "#about" },
    { name: "Experience", href: "#experience" },
    { name: "Projects", href: "#projects" },
    { name: "Skills", href: "#skills" },
    { name: "Education", href: "#education" },
    { name: "Contact", href: "#contact" },
  ];

  onMount(() => {
    const handleScroll = () => {
      scrolled = window.scrollY > 20;
    };
    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  });
</script>

<nav
  class="navbar fixed top-0 w-full z-50 transition-all duration-300 {scrolled
    ? 'bg-gray-50 dark:bg-gray-800 transition-colors duration-300 backdrop-blur-md shadow-lg'
    : 'bg-gray-50/50 dark:bg-gray-800/50 transition-colors duration-300 backdrop-blur-sm'}"
>
  <div class="navbar-start">
    <!-- Logo -->
    <a href="#home">
      <img src="/assets/favicon.png" alt="Logo" class="w-8 h-8 mx-4" />
    </a>
  </div>

  <!-- Desktop Navigation -->
  <div class="navbar-center hidden lg:flex">
    <ul class="menu menu-horizontal px-1">
      {#each navItems as item}
        <li>
          <a
            href={item.href}
            class="hover:text-primary text-gray-900 dark:text-white"
          >
            {item.name}
          </a>
        </li>
      {/each}
    </ul>
  </div>

  <!-- Right side buttons -->
  <div class="navbar-end gap-2">
    <!-- QR Code Button -->
    <button
      onclick={() => (showQR = true)}
      class="btn btn-primary btn-sm"
      aria-label="Show QR Code"
    >
      <svg
        class="w-5 h-5"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
        <path
          stroke-linecap="round"
          stroke-linejoin="round"
          stroke-width="2"
          d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"
        />
      </svg>
    </button>

    <!-- Theme Toggle with DaisyUI swap -->
    <label
      class="swap swap-rotate text-gray-900 dark:text-white"
      title={darkMode ? "Switch to light" : "Switch to dark"}
    >
      <input
        type="checkbox"
        checked={darkMode}
        onchange={toggleTheme}
        aria-label="Toggle theme"
		class="hidden"
      />

      <!-- moon icon (shown when dark mode is ON) -->
      <svg
        class="swap-on fill-current w-5 h-5"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
      >
        <path
          d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"
        />
      </svg>

      <!-- sun icon (shown when dark mode is OFF) -->
      <svg
        class="swap-off fill-current w-5 h-5"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
      >
        <path
          d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z"
        />
      </svg>
    </label>

    <!-- Mobile Menu Dropdown -->
    <div class="dropdown dropdown-end lg:hidden">
      <label tabindex="0" class="btn btn-ghost btn-sm">
        <svg
          class="w-6 h-6"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke-width="2"
            d="M4 6h16M4 12h16M4 18h16"
          />
        </svg>
      </label>
      <ul
        tabindex="0"
        class="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow-lg bg-white dark:bg-gray-800 transition-colors duration-300 rounded-box w-52"
      >
        {#each navItems as item}
          <li>
            <a
              href={item.href}
              class="hover:text-primary text-gray-900 dark:text-white"
            >
              {item.name}
            </a>
          </li>
        {/each}
      </ul>
    </div>
  </div>
</nav>
